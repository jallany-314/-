...
public class ClockView extends View {
    ...
    private void drawTimeNeedles(Canvas canvas) {
        ...
        float minuteDegree = (((float)minute + (float)second / 60) / 60) * 360 - 90;
        ...
        float secondDegree = ((float)second / 60) * 360 - 90;
    ...
    private void drawTimeNumbers(Canvas canvas) {
        // TODO 绘制表盘时间数字（可选）
        float length = radius * SECOND_NEEDLE_LENGTH_RATIO;
        for(int i = 0; i <= 11; i++){
            double x = centerX + length * Math.cos(Math.toRadians(i * 30 - 90));
            double y = centerY + radius * 0.05 + length * Math.sin(Math.toRadians(i * 30 - 90));
            String tmp = "";
            if(i < 10)
                tmp = tmp + "0" + Integer.toString(i);
            else
                tmp += Integer.toString(i);
            canvas.drawText(tmp, (float)x, (float)y, numberPaint);
        }
    }
